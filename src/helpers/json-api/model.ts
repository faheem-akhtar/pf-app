...
export class JsonApiModel {
   ...
   * JSON API resource type (must match the backend API resource type)
   */
  public jsonApiType: string = JsonApiModel.JSONAPI_TYPE;
  ...
  public serialize(
  ...
  ): JsonApiPayloadInterface {
    const data: JsonApiDataInterface = {
      id: '',
    ...
    relationships.forEach((key) => {
      ...
      if (!this[key]) {
        ...
        data.relationships[key as string] = {
          data: (this[key] as unknown as Array<JsonApiModel>).map((model: JsonApiModel) => {
            return {
              type: model.jsonApiType,
              id: model.id,
            };
          }) as JsonApiDataInterface[],
        };

        // Relationship is an object
      } else {
        const model = this[key] as unknown as JsonApiModel;
   ...
   */
  public setRelationship(relName: keyof this, models: JsonApiModel | JsonApiModel[] | null): void {
