...
describe('localeGetLangAwareHref', () => {
  ...
  it('should return the href with the current locale', () => {
    expect(localeGetLangAwareHref(current, path)).toEqual(`https://${configOriginValue}/${current}${path}`);
  });
  ...
  it('should support relative urls', () => {
    expect(localeGetLangAwareHref(current, path, true)).toEqual(`/${current}${path}`);
