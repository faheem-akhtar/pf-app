...
describe('PropertyCardComponent', () => {
  ...
  describe('Gallery', () => {
    ...
    it('should show placeholder image if the property has not included img url', () => {
      const defaultProps = makeDefaultProps();
      const { getByTestId } = render(
        <PropertyCardComponent
          {...defaultProps}
          property={propertyStub({
            [propertySerpObfuscatedFieldImgUrl]: '',
            [propertySerpObfuscatedFieldImagesCount]: 0,
          })}
        />
      );
      ...
      const galleryPlaceHolderImage = getByTestId('gallery-placeholder-image');

      expect(galleryPlaceHolderImage).toBeInTheDocument();
    });
  });
